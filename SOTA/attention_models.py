...
from tensorflow.keras.layers import multiply, Permute, Concatenate, Conv1D, Conv2D, Add, Activation, Lambda
...
def spa(input_feature,ratio=16):

	channel_axis = 1 if K.image_data_format() == "channels_first" else -1
	channel = input_feature.shape[channel_axis]

	spa_feature1 = GlobalAveragePooling2D()(input_feature)
	spa_feature1 = Reshape((1, 1, channel))(spa_feature1)

	spa_feature2 = GlobalAveragePooling2D()(input_feature)
	spa_feature2 = Reshape((1, 1, channel))(spa_feature2)

	spa_feature3 = GlobalAveragePooling2D()(input_feature)
	spa_feature3 = Reshape((1, 1, channel))(spa_feature3)

	spa_feature = Concatenate(axis=-1)([spa_feature1, spa_feature2, spa_feature3])

	spa_feature = Dense(channel // ratio,
					   activation='relu',
					   kernel_initializer='he_normal',
					   use_bias=True,
					   bias_initializer='zeros')(spa_feature)

	spa_feature = Dense(channel,
					   activation='sigmoid',
					   kernel_initializer='he_normal',
					   use_bias=True,
					   bias_initializer='zeros')(spa_feature)

	spa_feature = multiply([input_feature, spa_feature])
	return spa_feature

def scse(x):
	in_channel = x.shape[-1]
	y1 = Conv2D(filters=1, strides=1, kernel_size=1,bias_initializer='zeros')(x)
	print(y1.shape)
	y1 = Activation('sigmoid')(y1)

	y2 = GlobalAveragePooling2D()(x)
	print(y2.shape)
	y2 = Reshape((1, 1, in_channel))(y2)
	print(y2.shape)
	y2 = Conv2D(in_channel//2,kernel_size=1,bias_initializer="zeros")(y2)
	print(y2.shape)
	y2 = Conv2D(in_channel , kernel_size=1, bias_initializer="zeros")(y2)
	print(y2.shape)
	y2 = Activation('sigmoid')(y2)

	return multiply([x, y1])+multiply([x, y2])